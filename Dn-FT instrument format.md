...
| char           | 1                       | DPCM sample assignment count | `Note`                                          | DPCM sample assignment note index                                                        | 0 to `NOTE_COUNT`        | Off by +1. Written only when a sample exists at that note.     |                                 |
| char           | 1                       | ^                            | `CInstrument2A03::m_cSamples[Octave][Note]`     | DPCM sample assignment index                                                             | 0 to `MAX_DSAMPLES`      | If assignment is greater than `MAX_DSAMPLES`, it is not added. |                                 |
